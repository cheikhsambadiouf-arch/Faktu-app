// catalog-page.js — Page HTML publique du catalogue (/c/:slug). Le client
// choisit un produit deja enregistre par le vendeur, a prix fixe et
// garanti — pas de validation manuelle necessaire, il va directement au
// paiement une fois sa commande envoyee.

function renderCatalogPage(slug) {
  return `<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
...
async function submitOrder(){
  ...
  try{
    const res = await fetch(\`/api/public/catalog/\${SLUG}/order\`, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ product_id: chosenProduct.id, qty, client_name: name, client_phone: phone, client_address: address, preferred_delivery_date: date })
    });
